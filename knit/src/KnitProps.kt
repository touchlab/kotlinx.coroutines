...
val globalProperties = KnitProps()
...
class KnitProps(
    ...
    private fun loadTemplateLinesImpl(name: String, env: Any): List<String>? {
        ...
        // return all lines and also remove all extra blank lines
        return StringReader(writer.toString()).readLines().dropLastWhile { it.isBlank() }
    }
}

fun File.findProps(): KnitProps = absoluteFile.parentFile?.findDirProps() ?: globalProperties

private val propsCache = HashMap<File, KnitProps>()
private val rootPath = File(System.getProperty("user.dir")).absolutePath

private fun File.findDirProps(): KnitProps {
    if (!path.startsWith(rootPath)) return globalProperties
    propsCache[this]?.let { return it }
    val propFile = File(this, KNIT_PROPERTIES)
    if (!propFile.exists()) return parentFile.findDirProps()
    val props = KnitProps(DirectoryProps(this), parentFile?.findDirProps())
